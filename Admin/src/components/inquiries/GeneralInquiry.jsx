...
const GeneralInquiries = () => {
  ...
  useEffect(() => {
    getGenInqs();
    const intervalId = setInterval(getGenInqs, 1000);
  ...
  return (
    <div className="md:w-[85%]  w-11/12 bg-black/80 text-white font-urbanist ">
      ...
      <div className="pb-5">
        {inquires.length > 0 ? (
          <table className="min-w-full bg-black/90  border border-gray-600 rounded-lg">
            <thead className="sticky top-20 bg-black">
              <tr className="border-b text-center border-gray-700">
                <th className="px-4 py-4  text-[18px] font-semibold text-gray-200">
                  Name
                </th>
                <th className="px-4 py-4  text-[18px] font-semibold text-gray-200">
                  Email
            ...
            <tbody>
              {inquires.map((inq) => (
                ...
                >
                  ...
                  <td className="px-4 py-4  text-gray-300">{inq?.email}</td>
      ...
      {showModal && selectedRow && (
        <div className="fixed inset-0 bg-gradient-to-br from-black/80 via-gray-900/90 to-black/80 flex items-center justify-center z-50">
          <div
            className="bg-white/20 backdrop-blur-lg text-white p-8 rounded-3xl w-11/12 md:w-1/2 relative max-h-[90vh] shadow-2xl overflow-y-auto animate-fadeIn"
            style={{
