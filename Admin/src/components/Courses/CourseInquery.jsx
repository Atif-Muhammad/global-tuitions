...
const CourseInquiry = ({ onClose, data }) => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(getCourseInq(data._id), 10000);
  ...
  const handleReplyFlag = async (inq) => {
    ...
    try {
      Config.inqReply(payload)
        .then((res) => {
          setisReply(false);
          toast.success("Reply sent!"); // Success message using Toastify
          closeModal();
          getCourseInq(data._id);
        })
        .catch((err) => {
  ...
  return (
    <div className=" bg-black">
      <div className="fixed inset-0  font-urbanist backdrop-blur-md  bg-black bg-opacity-50 z-50">
        {inquires.length > 0 && (
          <h2 className="text-5xl bg-gray-800 text-white sticky top-0  font-urbanist px-4 py-6">
